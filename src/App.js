...
const reducer = (state, action) => {
  switch (action.type) {
    case "none":
      return state.map((el) =>
        el.id === action.id ? { ...el, status: "" } : el
      );
    case "WatchList":
      return state.map((el) =>
        el.id === action.id ? { ...el, status: "WatchList" } : el
      );

    case "WatchedList":
      return state.map((el) =>
        el.id === action.id ? { ...el, status: "WatchedList" } : el
      );
  ...
  return (
    <BrowserRouter>
      <MovieContext.Provider
        value={{ Contextdata: Moviedata, ContextDispatch: dispatch }}
      >
        <div className="page">
          <Navbar />
          <Routes>
